...
if exists('../out/position_summary.csv'):
    times = pd.read_csv('../out/position_summary.csv', index_col='Unnamed: 0')
else:
    times = pd.read_csv('../out/label_summary.csv', index_col='Unnamed: 0')
...
for timesIndex in times.index:
    if pd.isna(times.loc[timesIndex]['Closest']):
        t0 = time.time()
        ...
        if (distSaturn < 1e7):
            # Check Saturn first as kernel doesn't have other planets in Extended Extended mission
            closest = 'Saturn'
            dist = distSaturn
            pos = posSaturn
        else:
